...
[CustomEditor(typeof(TMGWaitingAreaDefinition))]
public class TMGWaitingAreaDefinition_Inspector : Editor {
	
	TMGWaitingAreaDefinition definition;
	
	private bool showLevelsEditor = true;
	
	public void OnEnable() {
		definition = (TMGWaitingAreaDefinition)target;
		
		if (definition.Levels == null) definition.Levels = new TMGWaitingAreaDefinition._Level[1];
	}
	...
	public override void OnInspectorGUI() {
		...
		if (showLevelsEditor) {
			...
			if(GUILayout.Button("Add Level")) {
				ArrayUtility.Add(ref definition.Levels, new TMGWaitingAreaDefinition._Level());
				GUI.changed = true;
			}
